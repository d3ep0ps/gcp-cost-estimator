...
async def test_mcp_parse_terraform_extracts_bigquery_dataset() -> None:
    """Verify that calling the parse_terraform tool returns a resource model with BigQuery dataset."""
    ...
    bq = next(r for r in resources if r["resource_id"] == "google_bigquery_dataset.with_location")
    assert bq["provider"] == "gcp"
    assert bq["service"] == "bigquery"
    assert bq["kind"] == "bigquery_dataset"


async def test_mcp_validate_resource_model_bigquery_defaults_applied() -> None:
    """Verify that calling validate_resource_model applies BigQuery defaults."""
    model_dict = {
        "resources": [
            {
                "provider": "gcp",
    ...
    assert norm_res["usage"]["active_storage_gb"] == 100.0
